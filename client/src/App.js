...
class App extends Component {
  ...
  questionTypeEnum = {
    ...
    checkedAnswers(){
      var answerIdList = this.getAnswerIdList();
      for(var i = 0; i < this.state.optionList.length; i++){
        var selectButtonId = "id_select_" + i;
        var checkedButton = document.getElementById(selectButtonId);
        if(answerIdList[i] === true){
    ...
    checkFormInput() {
      if(this.submitQuiz.course.length === 0){
        alert("Please input Question!");
        return false;
      }
      if(this.submitQuiz.author.length === 0){
        alert("Please input Author!");
        return false;
      }
      if(this.submitQuiz.question.length === 0){
        alert("Please input question!");
        return false;
      }
      if(this.submitQuiz.optionList.length === 0){
        alert("Please input options!");
        return false;
      }
      if(this.submitQuiz.answerList.length === 0){
    ...
    // onAnswerSelect(){
    //   var alist = this.getAnswerList();
    //   console.info("aaaaaaaaaaaalist : ", alist);
    //   this.setState({
    //     answerList: alist
    //   })
    // }
    ...
    submitQuizToServer() {
      ...
      fetch('/submitQuiz', {
        ...
        body: JSON.stringify(this.submitQuiz)
      })
      .then(function (res) {
    ...
    onSubmit() {
      this.submitQuiz.course = document.getElementById('id_course').value.trim();
      this.submitQuiz.author = document.getElementById('id_author').value.trim();
      this.submitQuiz.quizType = this.state.singleSelect;
      this.submitQuiz.question = document.getElementById('id_question').value.trim();
      this.submitQuiz.optionList = this.state.optionList;
      this.submitQuiz.answerList = this.getAnswerList();
      this.submitQuiz._id = this.state.quizId;
      console.log(this.submitQuiz);
